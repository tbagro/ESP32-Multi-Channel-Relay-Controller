...
//################ VARIABLES ################
const char* ssid = "Tamanduai";     // WiFi SSID     replace with details for your local network
const char* password = "1abacate";  // WiFi Password replace with details for your local network
...
void setup() {
  ...
  server.on("/schedule-setup", HTTP_GET, [](AsyncWebServerRequest* request) {
    String html = "<html><body>";
    html += "<form method='post'>";
    html += "<label for='schedule-system-functions'>SCHEDULE SYSTEM FUNCTIONS:</label><br>";
    html += "<textarea id='schedule-system-functions' name='schedule-system-functions' rows='10' cols='50'></textarea><br><br>";
    html += "<input type='submit' value='Submit'>";
    html += "</form></body></html>";
    request->send(200, "text/html", html);
  });

  server.on("/schedule-setup", HTTP_POST, [](AsyncWebServerRequest* request) {
    String functions = request->getParam("schedule-system-functions", true, true)->value();
    // process the functions here
    request->send(200, "text/plain", "Schedule system functions submitted");
  });
  ...
  server.on("/handlesetup", HTTP_GET, [](AsyncWebServerRequest* request) {
    if (request->hasArg("manualoverride1")) {
      ...
      if (stringArg == "ON") Channel1_Override = true;
      else Channel1_Override = false;
    }
    if (request->hasArg("manualoverride2")) {
      String stringArg = request->arg("manualoverride2");
      if (stringArg == "ON") Channel2_Override = true;
      else Channel2_Override = false;
    }
    if (request->hasArg("manualoverride3")) {
      String stringArg = request->arg("manualoverride3");
      if (stringArg == "ON") Channel3_Override = true;
      else Channel3_Override = false;
    }
    if (request->hasArg("manualoverride4")) {
      String stringArg = request->arg("manualoverride4");
      if (stringArg == "ON") Channel4_Override = true;
      else Channel4_Override = false;
    }
    SaveSettings();
    request->redirect("/homepage");  // Go back to home page
  });
...
void scheduleSetup() {
  Serial.println("Enter schedule in the following format:");
  Serial.println("SCHEDULE_TYPE HH:MM [DAY_OF_WEEK] [MONTH_DAY] [YEAR_MONTH_DAY]");
...
public:
  Schedule(String str) {
    // Parse string and set activation and deactivation times
  }

  void update() {
    // Check if it's time to activate or deactivate this schedule
  }
...
void processScheduleFunctions(String functions) {
  int count = 0;
  String line;
  while (functions.indexOf('\n', count) != -1) {
    line = functions.substring(count, functions.indexOf('\n', count));
    if (line.startsWith("SH")) {
      // Scheduled at a specific Date/Time On (High): SHyyyy-mm-dd hh:mm
      // set On on yyyy-mm-dd hh:mm
    } else if (line.startsWith("SL")) {
      // Scheduled at a specific Date/Time Off (Low): SLyyyy-mm-dd hh:mm
      // set Off on yyyy-mm-dd hh:mm
    } else if (line.startsWith("MH")) {
      // Monthly On (High): MHdd hh:mm
      // set On monthly on day dd hh:mm
    } else if (line.startsWith("ML")) {
      // Monthly Off (Low): MLdd hh:mm
      // set Off monthly on day dd hh:mm
    } else if (line.startsWith("WH")) {
      // Weekly On (High): WHd hh:mm 1-7
      // set On weekly on day d hh:mm
    } else if (line.startsWith("WL")) {
      // Weekly Off (Low): WLd hh:mm
      // set Off weekly on day d hh:mm
    } else if (line.startsWith("DH")) {
      // Daily On (High): DHhh:mm
      // set On daily at hh:mm
    } else if (line.startsWith("DL")) {
      // Daily Off (Low): DLhh:mm
      // set Off daily at hh:mm
    } else if (line.startsWith("IH")) {
      // Intervaled On (High): IHhh:mm
      // set On after being Off for hh:mm
  ...
  void CheckTimerEvent() {
    ...
    if (Channel1_State == "ON") ActuateChannel(ON, Channel1, Channel1_Pin);
    else ActuateChannel(OFF, Channel1, Channel1_Pin);
    if (Channel2_State == "ON") ActuateChannel(ON, Channel2, Channel2_Pin);
    else ActuateChannel(OFF, Channel2, Channel2_Pin);
    if (Channel3_State == "ON") ActuateChannel(ON, Channel3, Channel3_Pin);
    else ActuateChannel(OFF, Channel3, Channel3_Pin);
    if (Channel4_State == "ON") ActuateChannel(ON, Channel4, Channel4_Pin);
    else ActuateChannel(OFF, Channel4, Channel4_Pin);
  }
  //#########################################################################################
  void ActuateChannel(bool demand, byte channel, byte channel_pin) {
    if (demand) {
      if (ChannelReverse) digitalWrite(channel_pin, HIGH);
      else digitalWrite(channel_pin, LOW);
      Serial.println("Putting Channel-" + String(channel) + " ON");
    } else {
      if (ChannelReverse) digitalWrite(channel_pin, LOW);
      else digitalWrite(channel_pin, HIGH);
    ...
    if (Channel1_State == "ON" ||  // Switch ON LED if any channel is ON otherwise switch ot OFF
        Channel2_State == "ON" || Channel3_State == "ON" || Channel4_State == "ON") digitalWrite(LEDPIN, LOW);
  ...
  void StartSPIFFS() {
    ...
    if (SPIFFS_Status == false) {  // Most likely SPIFFS has not yet been formated, so do so
